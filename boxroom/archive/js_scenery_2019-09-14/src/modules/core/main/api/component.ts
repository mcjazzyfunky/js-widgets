...
import ComponentConfigA from './types/ComponentConfigA'
import ComponentConfigB from './types/ComponentConfigB'
import ComponentOptions from './types/ComponentOptions'

function component<P extends Props>
  (config: ComponentConfigA<P>): ExtFunctionComponent<P>

function component<P extends Props>
  (config: ComponentConfigB<P>): ExtFunctionComponent<P>

function component<P extends Props = {}>(
  displayName: string,
  renderer?: ComponentConfigA<P>['render'], // TODO
  options?: ComponentOptions
): ExtFunctionComponent<P>

function component<P extends Props = {}>(
  displayName: string,
  initializer?: ComponentConfigB<P>['init'], // TODO
  options?: ComponentOptions
): ExtFunctionComponent<P>
...
function component<P extends Props = {}>(
  ...
  return typeof arg1 === 'string'
    ...
    : buildComponent(arg1.displayName, arg1.render || arg1.init, arg1.validate, arg1.memoize)
}
...
if (process.env.NODE_ENV) {
  ...
  const validateComponentConfigA = Spec.exact({
  ...
  const validateComponentConfigB = Spec.exact({
    displayName: Spec.match(REGEX_DISPLAY_NAME),
    validate: Spec.optional(Spec.function),
    memoize: Spec.optional(Spec.boolean),
    init: Spec.function
  ...
  validateComponentConfig = Spec.or(
    {
      ...
      then: validateComponentConfigA
    }, {
      when: Spec.hasOwnProp('init'),
      then: validateComponentConfigB
    })
